...
export async function POST(req: NextRequest, { params }: { params: Promise<{ cid: string }> }) {
  try {
    ...
    if (actAs[0] !== calledDealer) {
      return NextResponse.json({ success: false, error: "Only the called dealer can post" }, { status: 403 });
    }

    const result = await submitExercise(actAs, TEMPLATE_IDS.MarginCallDemand, cid, "Post", {});
    ...
    return NextResponse.json({ success: true, demand_id: created?.contractId ?? cid, posted: true, update: result.update });
  } catch (e) {
