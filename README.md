...
Only the following Party IDs are authorized to connect (enforced by `middleware.ts` and `lib/canton/authorization.ts`, sourced from `lib/config/allowlist.ts`). All four are local parties on the 5N sandbox validator, and the OIDC M2M ledger user holds `CanActAs` rights on each. Use one of these when connecting a wallet in the UI:

| Role | Party ID |
|---|---|
| Dealer A (proposer) | `derive-dealerA::1220a14ca128063b8dc9d1ebb0bd22633be9f2168500f4dbc1ecaeb1855b14e5acf8` |
| Dealer B (acceptor) | `derive-dealerB::1220a14ca128063b8dc9d1ebb0bd22633be9f2168500f4dbc1ecaeb1855b14e5acf8` |
| Dealer C (novation incoming) | `derive-dealerC::1220a14ca128063b8dc9d1ebb0bd22633be9f2168500f4dbc1ecaeb1855b14e5acf8` |
| Regulator (disclosure) | `derive-regulator::1220a14ca128063b8dc9d1ebb0bd22633be9f2168500f4dbc1ecaeb1855b14e5acf8` |

These are the hardcoded defaults compiled into the app. To use a different set (e.g. for a different DevNet environment), override with `ALLOWED_PARTY_IDS` in `.env.local` (and in Vercel project env vars) as a semicolon-separated list. Parties must be local to the participant behind `CANTON_LEDGER_API_URL`, and the M2M ledger user must hold `CanActAs` rights on them.
